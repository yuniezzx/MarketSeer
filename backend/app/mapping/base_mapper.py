...
from ..data_sources.client_manager import ClientManager
...
class BaseMapper:
    ...
    def __init__(self, client_manager: ClientManager, field_mapping: Dict[str, List[Dict[str, Any]]], api_configs: Dict[str, Dict[str, Any]]):
        """
        初始化映射器
        ...
        for fallback_config in fallback_chain:
            ...
            try:
                ...
                if value is not None:
                    self.logger.info(f"Successfully fetched field '{model_field}' from {data_source}.{api_name}")
                    return value
                else:
                    self.logger.warning(f"Field '{source_field}' is None in response from {data_source}.{api_name}")
