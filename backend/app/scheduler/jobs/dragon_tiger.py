...
def collect_daily_active_brokerage(app):
    """
    采集每日活跃营业部数据

    在每个交易日收盘后执行，采集当天的活跃营业部数据
    ...
    with app.app_context():
        try:
            # 检查是否为交易日（周末和节假日不执行）
            if not is_trading_day():
                logger.info("今天不是交易日，跳过活跃营业部数据采集")
                return

            logger.info("=" * 60)
            logger.info("开始采集每日活跃营业部数据")
            logger.info("=" * 60)
            ...
            if success:
                logger.info("=" * 60)
                logger.info("每日活跃营业部数据采集完成")
                logger.info("=" * 60)
            else:
                logger.error("每日活跃营业部数据采集失败")
...
def register_jobs(scheduler, app):
    ...
    # 每个交易日 20:00 执行
    scheduler.add_job(
        func=collect_daily_active_brokerage,
        trigger="cron",
        args=[app],
        id="collect_daily_active_brokerage",
        name="采集每日活跃营业部数据",
        hour=20,
        minute=0,
        replace_existing=True,
    )
    logger.info("已注册任务: 采集每日活跃营业部数据 (每天 20:00)")
